...
namespace NameOf.Fody {
	public partial class ModuleWeaver {
		...
		private Boolean IsUsed(IMetadataTokenProvider methodDefinition) {
			try {
		...
		private Boolean IsUsed(FieldDefinition fieldDefinition) {
			try {
				return ModuleDefinition.GetTypes()
				                       .SelectMany(x => x.Methods)
				                       .SelectMany(x => x.Body.Instructions)
		...
		private static readonly OpCode[] LoadOpCodes =
			LoadFieldOpCodes.Concat(new[] {OpCodes.Ldloc, OpCodes.Ldloc_S, OpCodes.Ldloca, OpCodes.Ldloca_S, OpCodes.Ldloc_0, OpCodes.Ldloc_1, OpCodes.Ldloc_2, OpCodes.Ldloc_3}).ToArray();
