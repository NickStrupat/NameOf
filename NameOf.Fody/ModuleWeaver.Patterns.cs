...
namespace NameOf.Fody {
	partial class ModuleWeaver {
		#region Op-code groupings
		private static OpCode[] callOpCodes;
		private static OpCode[] CallOpCodes {
			get { return callOpCodes ?? (callOpCodes = new[] { OpCodes.Call, OpCodes.Calli, OpCodes.Callvirt }); }
		}

		private static OpCode[] loadFieldOpCodes;
		private static OpCode[] LoadFieldOpCodes {
			get { return loadFieldOpCodes ?? (loadFieldOpCodes = new[] { OpCodes.Ldfld, OpCodes.Ldsfld, OpCodes.Ldflda, OpCodes.Ldsflda }); }
		}

		private static OpCode[] loadOpCodes;
		private static OpCode[] LoadOpCodes {
			get { return loadOpCodes ?? (loadOpCodes = LoadFieldOpCodes.Concat(new[] { OpCodes.Ldloc, OpCodes.Ldloc_S, OpCodes.Ldloca, OpCodes.Ldloca_S, OpCodes.Ldloc_0, OpCodes.Ldloc_1, OpCodes.Ldloc_2, OpCodes.Ldloc_3 }).ToArray()); }
		}

		private static OpCode[] loadLocalObjectOpCodes;
		private static OpCode[] LoadLocalObjectOpCodes {
			get { return loadLocalObjectOpCodes ?? (loadLocalObjectOpCodes = new[] { OpCodes.Ldloc, OpCodes.Ldloc_S, OpCodes.Ldloca, OpCodes.Ldloca_S, OpCodes.Ldloc_0, OpCodes.Ldloc_1, OpCodes.Ldloc_2, OpCodes.Ldloc_3 }); }
		}

		private static OpCode[] loadArgumentObjectOpCodes;
		private static OpCode[] LoadArgumentObjectOpCodes {
			get { return loadArgumentObjectOpCodes ?? (loadArgumentObjectOpCodes = new[] { OpCodes.Ldarg, OpCodes.Ldarg_S, OpCodes.Ldarga, OpCodes.Ldarga_S, OpCodes.Ldarg_0, OpCodes.Ldarg_1, OpCodes.Ldarg_2, OpCodes.Ldarg_3 }); }
		}

		private static OpCode[] loadObjectOpCodes;
		private static OpCode[] LoadObjectOpCodes {
			get { return loadObjectOpCodes ?? (loadObjectOpCodes = LoadLocalObjectOpCodes.Concat(LoadArgumentObjectOpCodes).ToArray()); }
		}
		#endregion
		...
		private static PatternInstruction[][] nameOfCallPatterns = {
            ...
            #region State-machine IL from async/await
		    new [] {
                new PatternInstruction(OpCodes.Ldarg_0),
                new PatternInstruction(OpCodes.Ldfld, GetNameFromAsyncAwaitLocal),
		    ...
		    new [] {
                new PatternInstruction(OpCodes.Ldarg_0),
                new OptionalPatternInstruction(OpCodes.Ldftn),
                new PatternInstruction(CallOpCodes, (i, p) => ((MethodDefinition)i.Operand).Name, (i, p) => !((MethodDefinition)i.Operand).IsGetter),
                new OptionalPatternInstruction(OpCodes.Box),
                new NameOfPatternInstruction(),
            },
